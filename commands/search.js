...
module.exports = {
  name: "search",
  description: "Muestra resultados de la canción buscada",
  usage: "[canción]",
  permissions: {
  ...
  run: async (client, message, args, { GuildDB }) => {
    if (!message.member.voice.channel)
      return client.sendTime(
        message.channel,
        "❌ | **Debes estar en un canal de voz para escuchar algo!**"
      );
    if (
    ...
    if (Searched.loadType == "NO_MATCHES")
      ...
      let msg = await message.channel.send(
        "**Escribe el número de la canción que desea reproducir! Expira en `30 segundos`.**"
      );
      ...
      let SongID = await message.channel
        ...
        .catch(() => {
          er = true;
          msg.edit(
            "**Tardaste mucho en responder. Escribe de nuevo el comando para reproducir algo!**"
          );
        });
      if (er) return;
      /**@type {Message} */
      ...
      if (!parseInt(SongIDmsg.content))
        return client.sendTime(
          message.channel,
          "Por favor escribe correctamente el ID de la canción"
        );
      let Song = Searched.tracks[parseInt(SongIDmsg.content) - 1];
      if (!Song)
        return client.sendTime(
          message.channel,
          "No se encontró ninguna canción con esa ID"
        );
  ...
  SlashCommand: {
    options: [
      {
        ...
        type: 3,
        required: true,
        description: "Escribe la canción o url que deseas buscar",
      },
    ],
    /**
     *
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (
        ...
        return client.sendTime(
          interaction,
          ":x: | **Debes estar en el mismo canal que yo para usar ese comando!**"
        );
      let CheckNode = client.Manager.nodes.get(client.botconfig.Lavalink.id);
      if (!CheckNode || !CheckNode.connected) {
        return client.sendTime(
        ...
        switch (res.loadType) {
          ...
          case "SEARCH_RESULT":
            ...
            const resultss = new MessageEmbed()
              .setDescription(
                `${results}\n\n\t**Escribe el número de la canción que desea reproducir!**\n`
              )
              .setColor(client.botconfig.EmbedColor)
              .setAuthor(
            ...
            try {
              ...
              return awaitchannel.send(
                "❌ | **No has proporcionado una selección**"
              );
            }
