...
class DiscordMusicBot extends Client {
    ...
    this.Manager = new Manager({
      ...
      .on("trackStart", async (player, track) => {
        ...
        let TrackStartedEmbed = new MessageEmbed()
          .setAuthor(`Escuchando ♪`, this.botconfig.IconURL)
      ...
      if (err) this.log(err);
        ...
        files.forEach((file) => {
          ...
          if (!cmd.name || !cmd.description || !cmd.run)
            return this.log(
              "No se pudo cargar el comando: " +
                file.split(".")[0] +
                ", Razón: El archivo no tiene run/name/desciption"
            );
  ...
  sendError(Channel, Error) {
    let embed = new MessageEmbed()
      ...
      .setFooter(
        "Si piensas que esto es un bug, quéjese a Zeddy uwu"
      );
  ...
  build() {
    ...
    if (this.botconfig.ExpressServer) {
      this.http.listen(process.env.PORT || this.botconfig.Port, () =>
        this.log("Web Server se ha iniciado")
      );
    }
  }
